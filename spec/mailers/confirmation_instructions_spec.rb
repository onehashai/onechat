...
RSpec.describe 'Confirmation Instructions', type: :mailer do
  describe :notify do
    ...
    it 'does not refer to the inviter and their account' do
      expect(mail.body).to_not match('has invited you to try out OneChat!')
    end
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        expect(mail.body).to match(
          "#{CGI.escapeHTML(inviter_val.name)}, with #{CGI.escapeHTML(account.name)}, has invited you to try out OneChat!"
        )
      end
